...
#### Монохромные изображения (Grayscale)

| Смещение | Размер (байт) | Описание |
|:---------|:-------------:|:---------|
| 0 | 4 | rows (uint32) — высота изображения |
| 4 | 4 | cols (uint32) — ширина изображения |
| 8 | 4 | rank (uint32) — ранг приближения |
| 12 | rank × 4 | S (float32[rank]) — сингулярные значения |
| 12 + rank×4 | rows × rank × 4 | U (float32[rows×rank]) — левые сингулярные векторы |
| ... | rank × cols × 4 | Vt (float32[rank×cols]) — правые сингулярные векторы |

Общий размер файла: `12 + rank × (1 + rows + cols) × 4` байт

#### RGB изображения (3 канала)
...
| 4 | 4 | cols (uint32) — ширина изображения |
| 8 | 4 | rank (uint32) — ранг приближения |
| 12 | 4 | channels (uint32) — количество каналов (=3) |
| 16 | ... | Данные канала R: S, U, Vt |
| ... | ... | Данные канала G: S, U, Vt |
| ... | ... | Данные канала B: S, U, Vt |

Для каждого канала последовательно записываются:
- S (float32[rank]) — сингулярные значения
- U (float32[rows×rank]) — левые сингулярные векторы
- Vt (float32[rank×cols]) — правые сингулярные векторы

Общий размер файла: `16 + 3 × rank × (1 + rows + cols) × 4` байт
